...
import { matrixNormalize, downloadSvg } from "./helpers.js"
import Zoomer from "./zoomer__.js"
...
/*   setTimeout(()=>{
  ...
  await loadMapAndHotCreateBundleandUpdateSystem(SETTINGS.map)
});
...
async function loadMapAndHotCreateBundleandUpdateSystem(mapId){
    //first load map
  const response = await fetch(`svgmaps/${mapId}.svg`);
  if (!response.ok) {
    const message = `When downloading "${mapId}.svg", an Error has occured: ${response.status}`;
...
$("#select-map").chosen().change((event) => {
  loadMapAndHotCreateBundleandUpdateSystem(event.target.value)
});
